// This is the full React + Tailwind website code for YOUROWNWORTH (YOW)
// Project folder should be named: `YOW-website-code`

// File structure for GitHub:
// YOW-website-code/
// ├── public/
// │   └── assets/ (put your product images here)
// │       ├── hoodie-brown.jpg
// │       ├── hoodie-black.jpg
...
const handleBuy = async () => { try{ const res = await fetch('/.netlify/functions/create-checkout-session', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ priceId: p.id }) }); const json = await res.json(); if(json.url) window.location = json.url; }catch(err){ console.error(err); alert('Unable to start checkout.'); } };
return (
...
function App(){ return (<div className="font-sans"><Header /><Products /><Footer /></div>)}
...
// package.json (minimal setup)
{
  ...
  "devDependencies": {"vite": "^4.0.0", "tailwindcss": "^3.3.0", "postcss": "^8.4.0", "autoprefixer": "^10.4.0"}
}
...
// Now you can upload this folder to GitHub and connect it to Vercel for free hosting.
